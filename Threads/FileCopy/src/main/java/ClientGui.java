 ...
 * Purpose:
 * Example client for temperature conversion and file uploading.
 * A GUI window object extending JFrame. The GUI is for constructing
 * Extend this class and access its protected members to develop a Client.
 * The user selects the method name from the combo box
 * drop down list, provides values for up to 2 arguments and then selects
 * the <b>Call Method</b> button. The extending class handles the
 * button push, calls the appropriate method
 * and places any result in the <b>resultTextFld</b>.
 * <p>
...
public class ClientGui extends JFrame {
   ...
   /**
    * functionCB is the JComboBox containing the method names implemented by
    * the service.
    */
   protected JComboBox functionCB;
   /**
    * callFcnButt is the JButton for calling the selected function. The action
    * command is "Call Method".
    */
   protected JButton callFcnButt;
   /**
    * exitButt is the JButton for completing the application. The action
    * command is "Exit".
    */
   protected JButton exitButt;

   /**
    * Constructor for the function call client Gui. Creates a new
    * GUI object with with appropriate title and correct method names in
    * the drop-down combo box. Sets the window to be visible.
    * @param fcnNames a String array of the methods on the service that can
    * be invoked.
    * @param title a short title used to create a title line for the Gui
    * window's title.
    */
   public ClientGui (String[] fcnNames, String title) {
      super("Ser321 "+title+" Temperature Conversion Client");
