...
public class TiledRebusGameTCPServer {
    ...
    public static void main(String[] args) {
        ...
        final Option gridFactor = new Option("Pgrid", "gridFactor", true, "number of puzzle image partitions");

        cliOptions.addOption(port);
        cliOptions.addOption(gridFactor);

        CommandLineParser parser = new DefaultParser();
        HelpFormatter formatter = new HelpFormatter();
        CommandLine command;
        int parsedPort = 9000;
        int parsedGridFactor = 0;
        try {
            command = parser.parse(cliOptions, args);
            parsedPort = Integer.parseInt(command.getOptionValue(port.getOpt()));
            parsedGridFactor = Integer.parseInt(command.getOptionValue(gridFactor.getOpt()));
        ...
        try (ServerSocket listener = new ServerSocket(parsedPort)) {
            gameController = (parsedGridFactor == 0 ? new RebusPuzzleGameController() : new RebusPuzzleGameController(new PuzzleGame(RebusPuzzleGameController.convertGridDimensionToNumberOfQuestionsAvailable(parsedGridFactor)), parsedGridFactor));
            while (true) {
    ...
    private static final class Connection extends Thread {
        ...
        public void run() {
            ...
            try {
                do {
                    if (gameIsNull) {
                        outputStream.writeUTF(new Payload("Please enter an int >= 2", false).toString());
                        parsedPayload = parsePayload(inputStream.readUTF());
                        gameIsNull = currentGameIsNull(parsedPayload);
                    }

                    if (!gameIsNull) {
                        Payload questionOut = new Payload(gameController.getCurrentQuestion().getQuestion(), false);
        ...
        private Payload play(Payload playerResponse) {
            ...
            final String answer = playerResponse.getMessage();
            boolean answeredCorrectly = gameController.getCurrentGame().answerPuzzleQuestion(currentQuestion, answer);
            ...
            if (gameController.getCurrentGame().getNumberOfQuestionsAnsweredIncorrectly() ==
                ...
                return new Payload("Terribly sorry, but you have lost the game.", false);
            } else if (gameController.getCurrentGame().getRandomlySelectedRebus().isCorrect(answer)) {
        ...
        private boolean currentGameIsNull(Payload parsedPayload) throws IOException {
            try {
                final int gridDimension = Integer.parseInt(parsedPayload.getMessage());
                ...
                outputStream.writeUTF((new Payload(message, false)).toString());
                return true;
            }
        }
        ...
        public Payload initializeRebusPuzzleGameRequest() {
            ...
            return new Payload(message, false);
        }
